...
$id = (int) ($_GET['id'] ?? 0);
$editing = $id ? db()->prepare('SELECT * FROM categories WHERE id = ?') : null;
$category = null;
if ($editing) {
    $editing->execute([$id]);
...
<div class="row g-4">
    <div class="col-lg-4">
        <form class="bg-white rounded-2 p-4 shadow-sm" method="post">
            ...
            <input class="form-control mb-3" name="slug" value="<?= e($category['slug'] ?? '') ?>" placeholder="backend-php">
    ...
    <div class="col-lg-8">
        <div class="bg-white rounded-2 shadow-sm table-responsive">
            <table class="table mb-0">
                <thead><tr><th>Tên</th><th>Slug</th><th>Khóa học</th><th></th></tr></thead>
                <tbody>
